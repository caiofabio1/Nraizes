...
class EANFinder:
    ...
    def infer_ean_with_ai(
        self, product: Dict[str, Any], retry_count: int = 0
    ) -> List[EANCandidate]:
...
'''
            ...
            if json_match:
                ...
                if ean:
                    ...
                    if is_valid and corrected_ean:
                        ...
                        if brand == "Ocean Drop" and corrected_ean.startswith(
                            "7898681"
                        ):
                            confidence = min(confidence + 0.1, 0.99)
                        elif brand in ["Granado", "Phebo"] and corrected_ean.startswith(
                            "7891027"
                        ):
                            confidence = min(confidence + 0.1, 0.99)
    ...
    def find_eans_batch(
        self, products: List[Dict], limit: int = None
    ) -> Dict[int, List[EANCandidate]]:
