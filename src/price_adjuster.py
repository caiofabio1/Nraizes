...
class PriceAdjuster:
    ...
    def _load_config(self):
        ...
        self.min_margin = float(
            self.db.get_config("MIN_MARGIN_PERCENT") or self.DEFAULT_MIN_MARGIN
        )
        self.max_swing = float(
            self.db.get_config("MAX_PRICE_SWING_PERCENT") or self.DEFAULT_MAX_SWING
        )

    def _get_regra_produto(
        self, id_produto: int, marca: str = None, categoria: str = None
    ) -> Dict:
    ...
    def analisar_produto(self, id_produto: int) -> Optional[PriceRecommendation]:
        ...
        if diferenca_percent > premium_permitido:
            ...
            preco_sugerido = max(
                preco_mercado * (1 + premium_permitido / 100), preco_minimo
            )
        ...
        pode_auto = (
            permite_auto
            and self._pode_ajustar(id_produto)
            and acao != PriceAction.MAINTAIN
        )
    ...
    def analisar_todos(
        self, apenas_com_dados: bool = True
    ...
    def aplicar_ajuste(
        ...
        if bling_client:
            try:
                ...
                print(
                    f"✅ Preço atualizado no Bling: {produto['nome'][:30]} R${preco_anterior:.2f} → R${preco_novo:.2f}"
                )
                return True
        ...
        print(
            f"📝 Ajuste registrado localmente: {produto['nome'][:30]} R${preco_anterior:.2f} → R${preco_novo:.2f}"
        )
        return True
    ...
    def gerar_relatorio(self) -> str:
        ...
        if reducoes:
            ...
            for r in reducoes[:10]:
                linhas.append(
                    f"- **{r.nome_produto[:40]}**: R${r.preco_atual:.2f} → R${r.preco_sugerido:.2f} ({r.diferenca_percent:+.0f}%)"
                )
            linhas.append("")
