...
class ProductResearcher:
    ...
    def categorize_product(self, nome: str) -> str:
        ...
        elif any(x in nome_lower for x in ['cha ', 'chá ', 'folha', 'raiz', 'casca', 'erva']):
            return 'planta'
        elif any(x in nome_lower for x in ['capsulas', 'cápsulas', 'vitamina', 'proteina', 
                                            'colageno', 'omega', 'aminnu', 'guardian', 
                                            'magnesio', 'suplemento']):
            return 'suplemento'
        elif any(x in nome_lower for x in ['sabonete', 'shampoo', 'creme', 'serum', 
                                            'base', 'batom', 'mascara', 'oleo facial']):
            return 'cosmetico'
        elif any(x in nome_lower for x in ['mel', 'cafe', 'cha', 'shake', 'polpa', 
                                            'pao de queijo', 'torta', 'wafel']):
            return 'alimento'
        else:
            return 'outro'
...
'''
    ...
    def get_product_knowledge(self, id_produto: int) -> Optional[Dict[str, Any]]:
        ...
        cursor.execute('SELECT * FROM produto_conhecimento WHERE id_produto = ?', (id_produto,))
        row = cursor.fetchone()
        conn.close()
        ...
        # Parse JSON fields
        json_fields = ['ingredientes', 'principios_ativos', 'contraindicacoes', 
                       'interacoes', 'efeitos_colaterais', 'alertas', 'beneficios',
                       'indicacoes', 'certificacoes', 'referencias_cientificas', 'faq']
