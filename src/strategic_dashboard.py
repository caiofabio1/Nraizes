...
class MetricsCollector:
    ...
    def collect_daily_metrics(self) -> Dict[str, Any]:
        ...
        cursor.execute('SELECT COUNT(*) as total FROM produtos WHERE situacao = "A" AND (preco IS NULL OR preco = 0)')
        metrics['produtos_sem_estoque'] = cursor.fetchone()['total']
        
        # Propostas pendentes
        cursor.execute('SELECT COUNT(*) as total FROM propostas_ia WHERE status = "pendente"')
        metrics['propostas_pendentes'] = cursor.fetchone()['total']
        
        # Produtos sem EAN (gtin NULL)
        cursor.execute('SELECT COUNT(*) as total FROM produtos WHERE situacao = "A" AND (gtin IS NULL OR gtin = "")')
        row = cursor.fetchone()
        metrics['produtos_sem_ean'] = row['total'] if row else 0
        
        # Alertas de preço pendentes
        cursor.execute('SELECT COUNT(*) as total FROM alertas_preco WHERE status = "pendente"')
        row = cursor.fetchone()
        metrics['alertas_preco_pendentes'] = row['total'] if row else 0
        
        # Preços coletados (últimos 7 dias)
        cutoff = (datetime.now() - timedelta(days=7)).isoformat()
        cursor.execute('SELECT COUNT(DISTINCT id_produto) as total FROM precos_concorrentes WHERE coletado_em > ?', (cutoff,))
        row = cursor.fetchone()
        ...
        ''')
        row = cursor.fetchone()
        metrics['margem_media'] = round(row['margem_media'], 1) if row and row['margem_media'] else None
...
class GeminiAnalyzer:
    """Interface com Gemini Pro para análises estratégicas."""
...
'''
    ...
    def generate_report(self, analysis: Dict, snapshot: Dict) -> str:
        ...
        for insight in analysis.get('insights', []):
            icon = "✅" if insight.get('tipo') == 'positivo' else "⚠️" if insight.get('tipo') == 'negativo' else "ℹ️"
            lines.append(f"- {icon} **{insight.get('titulo')}**: {insight.get('descricao')}")
        
        lines.extend(["", "## 📈 Tendências"])
        for trend in analysis.get('tendencias', []):
            icon = "🔺" if trend.get('direcao') == 'up' else "🔻" if trend.get('direcao') == 'down' else "➡️"
            lines.append(f"- {icon} **{trend.get('metrica')}**: {trend.get('descricao')}")
        
        lines.extend(["", "## ✅ Ações Prioritárias"])
        for acao in sorted(analysis.get('acoes', []), key=lambda x: x.get('prioridade', 3)):
            lines.append(f"{acao.get('prioridade', '-')}. **[{acao.get('tipo', 'geral').upper()}]** {acao.get('acao')}")
            lines.append(f"   - Impacto: {acao.get('impacto')}")
        
        risco = analysis.get('risco', {})
        if risco:
            nivel_icon = "🔴" if risco.get('nivel') == 'alto' else "🟡" if risco.get('nivel') == 'medio' else "🟢"
            lines.extend([
                "",
...
class StrategicDashboard:
    ...
    def get_latest_report_data(self) -> Optional[Dict]:
        ...
        return {
            ...
            'insights': json.loads(report['insights_json']) if report['insights_json'] else [],
            'actions': json.loads(report['acoes_recomendadas']) if report['acoes_recomendadas'] else []
        }
    ...
    def _get_approval_candidates(self):
        ...
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        cursor.execute("SELECT id_bling, nome, gtin FROM produtos WHERE situacao='A' AND gtin IS NOT NULL AND gtin != ''")
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="md:flex md:items-center md:justify-between mb-8">
            <div class="flex-1 min-w-0">
                ...
                <p class="mt-1 text-sm text-gray-500">
                    Gerado em {datetime.strptime(report['created_at'], '%Y-%m-%d %H:%M:%S').strftime('%d/%m/%Y às %H:%M')}
                </p>
            </div>
        ...
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 mb-8">
            <div class="bg-white overflow-hidden shadow rounded-lg px-4 py-5 sm:p-6">
                ...
                <dd class="mt-1 text-3xl font-semibold text-gray-900">{metrics.get('produtos_ativos', 0)}</dd>
            </div>
             <div class="bg-white overflow-hidden shadow rounded-lg px-4 py-5 sm:p-6">
                <dt class="text-sm font-medium text-gray-500 truncate">Margem Média</dt>
                <dd class="mt-1 text-3xl font-semibold { 'text-red-600' if metrics.get('margem_media', 0) < 0 else 'text-green-600' }">
                    {metrics.get('margem_media', 0)}%
                </dd>
            </div>
            <div class="bg-white overflow-hidden shadow rounded-lg px-4 py-5 sm:p-6">
                <dt class="text-sm font-medium text-gray-500 truncate">Sem Estoque</dt>
                <dd class="mt-1 text-3xl font-semibold text-gray-900">{metrics.get('produtos_sem_estoque', 0)}</dd>
            </div>
            <div class="bg-white overflow-hidden shadow rounded-lg px-4 py-5 sm:p-6">
                <dt class="text-sm font-medium text-gray-500 truncate">Sem EAN</dt>
                <dd class="mt-1 text-3xl font-semibold text-gray-900">{metrics.get('produtos_sem_ean', 0)}</dd>
            </div>
        </div>
                    ...
                    <div class="overflow-y-auto max-h-96">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                </tr>''' for r in price_recs])}
                            </tbody>
                        </table>
                        { '<p class="text-sm text-gray-500 mt-2 text-center">Nenhuma sugestão de preço pendente.</p>' if not price_recs else '' }
                    </div>
                </div>
                    ...
                    <div class="overflow-y-auto max-h-96">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                </tr>''' for e in ean_recs[:100]])} 
                                <!-- Limit to 100 to avoid huge DOM -->
                            </tbody>
                        </table>
                         { f'<p class="text-xs text-gray-400 mt-2 text-center">Mostrando 100 de {len(ean_recs)} itens.</p>' if len(ean_recs) > 100 else '' }
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="bg-white shadow sm:rounded-lg">
                ...
                <ul class="divide-y divide-gray-200">
                    ...
                    </li>''' for a in sorted(actions, key=lambda x: x.get('prioridade', 9))])}
                </ul>
            </div>
        </div>
        ...
        <div class="mt-8 bg-white shadow sm:rounded-lg">
            ...
            <div class="px-4 py-5 sm:p-6 prose max-w-none text-gray-700">
                <pre class="whitespace-pre-wrap font-sans text-sm">{report['conteudo']}</pre>
            </div>
        </div>
    </div>
</body>
