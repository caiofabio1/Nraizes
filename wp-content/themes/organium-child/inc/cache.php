...
function nraizes_cache_category_query($q, $query) {
    ...
    if (!is_product_category()) return $q;
    
    $term = get_queried_object();
    if (!$term) return $q;
    
    $cache_key = 'nraizes_cat_' . $term->term_id . '_' . $q->get('paged');
...
function nraizes_cache_nav_menu($output, $args) {
    if (is_admin() || is_customize_preview()) return $output;
...
function nraizes_store_menu_cache($nav_menu, $args) {
    if (is_admin() || is_customize_preview()) return $nav_menu;
...
function nraizes_maybe_serve_cached_sidebar($has_widgets, $sidebar_id) {
    if (is_admin() || is_customize_preview()) return $has_widgets;
    
    // Only cache specific sidebars
    $cacheable = array('shop-sidebar', 'product-sidebar', 'sidebar-1');
    if (!in_array($sidebar_id, $cacheable)) return $has_widgets;
