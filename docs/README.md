...
## Integrações Python (Bling & Gestão Click)

O projeto inclui scripts Python para integração com APIs de ERP.

### Estrutura

```
src/
├── bling_client.py    # Cliente API Bling (v3)
└── gestao_client.py   # Cliente API Gestão Click
```

### Configuração

1. As credenciais devem estar em `.credentials/` (não versionado):
   - `.credentials/bling_api_tokens.env`
   - `.credentials/gestao_api_tokens.env`

### Uso

Scripts de verificação e uso estão disponíveis:
